  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <h2 className="text-2xl font-bold text-[#183964]">Career Analytics</h2>
  ...
  if (!analytics) {
    return (
      <div className="text-center py-20">
        <p className="text-[#6b7280]">Failed to load analytics. Make sure the server is running.</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-[#183964] flex items-center gap-3">
          <BarChart3 className="w-7 h-7 text-[#f36c21]" />
          Career Analytics
        </h2>
        <button
          onClick={fetchAnalytics}
          className="px-4 py-2 bg-[var(--color-dark-card)] border border-[var(--color-dark-border)] text-[#4b5563] rounded-lg hover:border-[#f36c21]/50 transition-colors text-sm"
        >
          Refresh
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((card) => (
          ...
          >
            ...
            <p className="text-3xl font-bold text-[#183964]">{card.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[var(--color-dark-card)] p-6 rounded-2xl border border-[var(--color-dark-border)]">
          <h3 className="text-lg font-semibold text-[#183964] mb-6">Hiring Funnel</h3>
          <div className="space-y-3">
            {statusBreakdown.map((item) => {
              ...
              return (
                <div key={item._id} className="flex items-center gap-3">
                  <span className="text-sm text-[#6b7280] w-24 shrink-0">{item._id}</span>
                  <div className="flex-1 h-8 bg-[#f7f9fc] rounded-lg overflow-hidden">
                    <div
                    ...
                    >
                      <span className="text-xs font-semibold text-[#183964]">{item.count}</span>
                    </div>
                  </div>
                </div>
              );
            ...
            {statusBreakdown.length === 0 && (
              <p className="text-[#6b7280] text-sm text-center py-4">No applications yet</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-[var(--color-dark-card)] p-6 rounded-2xl border border-[var(--color-dark-border)]">
          <h3 className="text-lg font-semibold text-[#183964] mb-6">Top Departments</h3>
          <div className="space-y-3">
            {departmentBreakdown.map((item, idx) => {
              ...
              return (
                <div key={item._id} className="flex items-center gap-3">
                  <span className="text-sm text-[#6b7280] w-32 shrink-0 truncate">{item._id}</span>
                  <div className="flex-1 h-7 bg-[#f7f9fc] rounded-lg overflow-hidden">
                    <div
                      className="h-full rounded-lg flex items-center px-3 transition-all duration-500"
                      style={{
            ...
            {departmentBreakdown.length === 0 && (
              <p className="text-[#6b7280] text-sm text-center py-4">No data yet</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-[var(--color-dark-card)] p-6 rounded-2xl border border-[var(--color-dark-border)]">
        <h3 className="text-lg font-semibold text-[#183964] mb-6">Applications Over Time</h3>
        {monthlyApplications.length === 0 ? (
          <p className="text-[#6b7280] text-sm text-center py-8">No application data yet</p>
        ) : (
