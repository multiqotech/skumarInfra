    ...
    <div className="min-h-screen bg-[var(--color-dark)] flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md bg-[var(--color-dark-card)] border border-[var(--color-dark-border)] rounded-3xl p-8 md:p-10 shadow-2xl z-10 relative overflow-hidden">
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <User className="h-5 w-5 text-gray-500 group-focus-within:text-[#FFB800] transition-colors" />
              </div>
              <input
                type="text"
                value={name}
        ...
        <div className="mt-8 text-center text-sm text-gray-400">
          ...
          <Link href={`/login${redirect !== '/' ? `?redirect=${encodeURIComponent(redirect)}` : ''}`} className="text-[#FFB800] hover:underline font-medium transition-colors">
            Sign In Here
          </Link>
        </div>
      </div>
